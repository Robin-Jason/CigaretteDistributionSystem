...
| 层级              | 主要包路径前缀                 | 角色定位                                             |
|-------------------|--------------------------------|------------------------------------------------------|
| API 层            | `org.example.api.web`          | 对外 HTTP 接口入口，面向前端/外部系统               |
| Application 层    | `org.example.application`      | 用例编排与应用服务层，组织完整业务流程             |
| Domain 层         | `org.example.domain`           | 领域模型与业务规则层，承载核心业务逻辑              |
| Infrastructure 层 | `org.example.infrastructure`   | 技术实现层：数据库、配置、监控、历史算法实现等     |
| Shared 层         | `org.example.shared`           | 跨层共享的基础能力：常量、工具、异常处理等         |
...
- **包含内容**：
  - `service/*`：按子域组织的应用服务接口与实现（calculate/importing/coordinator/encode/query）。
  - `orchestrator/*`：策略编排与算法编排/算法引擎。
  - `facade`：用例级的统一门面。
  - `dto` / `converter`：在 API 与 Domain/Infrastructure 之间传递和转换数据。
  - `event.handler`：对领域/应用事件的响应处理。
- **可以做的事**：
  - 组合多个领域服务和仓储，完成完整的业务用例。
  - 处理事务边界（通常通过 Service/Orchestrator 进行）。
  - 调用 Infrastructure 层的实现（通常通过 Domain 的 Repository 抽象或特定技术组件）。
  - 处理错误恢复、重试、补偿等用例级逻辑。
- **不应该做的事**：
  - 自己实现核心业务规则或算法（应该放在 Domain）。
...
- **包含内容**：
  - `model.entity` / `valueobject` / `tag`：实体与值对象。
  - `service.algorithm` / `service.rule` / `service.delivery`：领域服务与业务规则。
  - `repository`：仓储接口抽象。
  - `event`：领域事件。
- **可以做的事**：
...
- **包含内容**：
  - `persistence.mapper` / `persistence.po`：MyBatis Mapper 与 PO。
...
| 8        | `SingleLevelDistributionService` 等     | Domain           | 领域层算法服务，执行核心分配算法逻辑                                                               |
| 9        | `CigaretteDistributionPredictionRepository` 等 | Domain    | 提供对预测/分配结果的读写抽象接口                                                                  |
| 10       | `*RepositoryImpl` + `*Mapper` + `*Po`   | Infrastructure   | 通过 MyBatis 将领域仓储操作落地到数据库                                                             |
| 11       | `DistributionWriteBackService`          | Application      | 将计算后的矩阵写回预测表，并做必要统计/记录                                                         |
| 12       | `GenerateDistributionPlanResponseDto/Vo`| Application / API| 组装返回结果 DTO/VO，并由 Controller 封装成统一响应返回前端                                         |
...
| 日志配置（如 logback）         | 日志输出格式与级别控制               | 保证关键链路（如算法执行）日志可追踪                                             |
