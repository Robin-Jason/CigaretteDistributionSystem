...
| 步骤 | 代码位置                                      | 输入数据                                     | 本步主要动作                                                                                   | 输出/流向                                      |
|------|-----------------------------------------------|----------------------------------------------|------------------------------------------------------------------------------------------------|-----------------------------------------------|
| 7    | `application/service/writeback/impl/PriceBandDistributionWriteBackServiceImpl.writeBackPriceBandAllocations` | 候选卷烟列表（含 GRADES）+ year/month/weekSeq + 全市客户数数组 | 将分配结果转换为持久化对象，批量写入 `cigarette_distribution_prediction_price` 分区表，计算实际投放量 | 数据库持久化完成                               |
...
ExcelImportController
  │  组装 DataImportRequestDto
  ▼
ExcelImportServiceImpl.importData
  │  调用 importCigaretteDistributionInfo 解析 Excel
  │  验证文件类型 / 列结构
  │  调用 CigaretteImportValidator.validate 执行业务合法性校验
  │    - 校验全市投放占比是否达到阈值
  │    - 校验货源属性 + 投放类型 + 标签组合是否合法
  │  校验失败：捕获 IllegalArgumentException，返回 success=false、message="导入校验失败: xxx"
  │  校验通过：写入 cigarette_distribution_info 分区表
  ▼
数据库（cigarette_distribution_info / base_customer_info 等）
  │
  ▼
Controller 封装响应 → 前端（根据 success/message 提示导入结果）
```
