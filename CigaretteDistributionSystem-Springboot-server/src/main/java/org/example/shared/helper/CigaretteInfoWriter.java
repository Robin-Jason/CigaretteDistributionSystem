...
public class CigaretteInfoWriter {
    ...
    public int writeToPartition(List<Map<String, Object>> data,
        ...
        QueryWrapper<CigaretteDistributionInfoPO> qw = new QueryWrapper<>();
        qw.eq("YEAR", year).eq("MONTH", month).eq("WEEK_SEQ", weekSeq);
        int deleted = cigaretteDistributionInfoRepository.delete(qw);
        log.info("清空卷烟投放基础信息分区: {}-{}-{}, 删除 {} 条旧记录", year, month, weekSeq, deleted);

        List<CigaretteDistributionInfoPO> dataList = new ArrayList<>();
        for (Map<String, Object> row : data) {
            CigaretteDistributionInfoPO infoData = mapRow(row);
            if (infoData != null) {
        ...
        int inserted = cigaretteDistributionInfoRepository.batchUpsert(dataList);
        log.info("导入卷烟投放基础信息到分区表完成: {}-{}-{}, 插入 {} 条记录", year, month, weekSeq, inserted);
        return inserted;
    }

    private CigaretteDistributionInfoPO mapRow(Map<String, Object> row) {
        CigaretteDistributionInfoPO data = new CigaretteDistributionInfoPO();
