...
import org.example.application.facade.DistributionStrategyCommand;
import org.example.application.facade.DistributionStrategyManager;
...
public class DistributionAllocationOrchestrator {
     ...
     * @example
     * <pre>
     *     AllocationCalculationResult r = orchestrator.calculateAllocationMatrix(
     *         "42010020","红金龙(硬神州腾龙)","按档位扩展投放","档位+区县+市场类型",null,
     *         "丹江（城网）", new BigDecimal("51230"), 2025, 9, 3,
     *         advRow, new HashMap<String, BigDecimal>() {{
     *             put("urbanRatio", new BigDecimal("0.6"));
     *             put("ruralRatio", new BigDecimal("0.4"));
     *         }}, null
     *     );
     * </pre>
     */
    public AllocationCalculationResult calculateAllocationMatrix(String cigCode,
        ...
        try {
            ...
            DistributionStrategyCommand command = DistributionStrategyCommand.builder()
                    ...
                    .maxGrade(resolveGrade(advData.get("highest_grade"), "D30"))
                    .minGrade(resolveGrade(advData.get("lowest_grade"), "D1"))
                    .build();

            StrategyExecutionResult executionResult = strategyManager.executeDistributionStrategy(command);
    ...
    private Map<String, Object> buildExtraInfo(String deliveryEtype,
        ...
        if ("档位+市场类型".equals(deliveryEtype) && marketRatios != null) {
            ...
            if (urbanRatioParam != null && ruralRatioParam != null) {
                ...
                extraInfoMap.put(DistributionAlgorithmEngine.EXTRA_GROUP_RATIOS, groupRatios);
