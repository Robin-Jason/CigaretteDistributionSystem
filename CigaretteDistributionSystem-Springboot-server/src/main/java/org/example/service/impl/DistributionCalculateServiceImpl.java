...
import org.example.service.query.PartitionPredictionQueryService;
import org.example.mapper.CigaretteDistributionInfoMapper;
import org.example.mapper.RegionCustomerStatisticsMapper;
import org.example.service.util.DistributionDataConverter;
import org.example.service.orchestrator.AllocationCalculationResult;
import org.example.service.orchestrator.DistributionAllocationOrchestrator;
import org.example.service.impl.DistributionWriteBackService;
...
public class DistributionCalculateServiceImpl implements DistributionCalculateService {
    ...
    @Autowired
    private CigaretteDistributionInfoMapper cigaretteDistributionInfoMapper;

    @Autowired
    private RegionCustomerStatisticsMapper regionCustomerStatisticsMapper;
    ...
    private BigDecimal[] getCustomerCountsForTargetDynamic(String target, String deliveryMethod, String deliveryEtype,
        ...
        try {
            ...
            Map<String, Object> row = regionCustomerStatisticsMapper.findByRegion(year, month, weekSeq, target);
            if (row == null) {
    ...
    public GenerateDistributionPlanResponseDto generateDistributionPlan(GenerateDistributionPlanRequestDto request) {
        ...
        try {
            // 1. 检查指定日期是否存在分配数据
            List<CigaretteDistributionPredictionData> existingData = 
            ...
            if (!existingData.isEmpty()) {
                ...
                if (deletedCountObj instanceof Long) {
                    response.setDeletedRecords(((Long) deletedCountObj).intValue());
                } else if (deletedCountObj instanceof Integer) {
                    response.setDeletedRecords((Integer) deletedCountObj);
                } else {
                    response.setDeletedRecords(deletedCountObj != null ? Integer.valueOf(deletedCountObj.toString()) : 0);
                }
            } else {
            ...
            try {
                ...
                QueryWrapper<CigaretteDistributionInfoData> infoQuery = new QueryWrapper<>();
                infoQuery.eq("YEAR", request.getYear())
                        .eq("MONTH", request.getMonth())
                        .eq("WEEK_SEQ", request.getWeekSeq())
                        .orderByAsc("CIG_CODE", "CIG_NAME");
                List<Map<String, Object>> advDataList = cigaretteDistributionInfoMapper.selectMaps(infoQuery);
                log.info("从分区表获取{}年{}月第{}周的预投放量数据数量: {}", 
                        request.getYear(), request.getMonth(), request.getWeekSeq(), advDataList.size());
                ...
                int successCount = 0;
                int totalCount = 0;
                long totalCalcTime = 0L;
                long totalWriteBackTime = 0L;
                long maxCalcTime = 0L;
                long maxWriteTime = 0L;
                String slowestCalcCig = null;
                String slowestWriteCig = null;
                ...
                for (Map<String, Object> advData : advDataList) {
                    ...
                    try {
                        if (deliveryArea != null && !deliveryArea.trim().isEmpty()) {
                            ...
                            totalCalcTime += calcElapsed;
                            if (calcElapsed > maxCalcTime) {
                                maxCalcTime = calcElapsed;
                                slowestCalcCig = cigCode + "-" + cigName;
                            }
                            cigResult.put("calcTimeMs", calcElapsed);
                            ...
                            if (allocationCalcResult.isSuccess() && !allocationCalcResult.getTargetList().isEmpty() 
                                ...
                                totalWriteBackTime += writeElapsed;
                                if (writeElapsed > maxWriteTime) {
                                    maxWriteTime = writeElapsed;
                                    slowestWriteCig = cigCode + "-" + cigName;
                                }
                                cigResult.put("writeBackTimeMs", writeElapsed);
                ...
                long avgCalc = totalCount > 0 ? totalCalcTime / totalCount : 0L;
                long avgWrite = writeAttemptCount > 0 ? totalWriteBackTime / writeAttemptCount : 0L;
                log.info("分配矩阵写回完成，成功: {}/{}", successCount, totalCount);
                log.info("【性能】算法耗时统计: total={}ms, avg={}ms, max={}ms({})", 
                        totalCalcTime, avgCalc, maxCalcTime, slowestCalcCig);
                log.info("【性能】写回耗时统计: total={}ms, avg={}ms, max={}ms({})",
                        totalWriteBackTime, avgWrite, maxWriteTime, slowestWriteCig);
            } catch (Exception e) {
            ...
            if (Boolean.TRUE.equals(allocationResult.get("success"))) {
                // 4. 分配成功，查询生成的分配记录数
                List<CigaretteDistributionPredictionData> generatedData = 
                ...
                // 分配失败，但仍需统计可能已生成的记录数
                List<CigaretteDistributionPredictionData> partialData = 
                    partitionPredictionQueryService.queryPredictionByTime(
                        request.getYear(), request.getMonth(), request.getWeekSeq());
            ...
            try {
                List<CigaretteDistributionPredictionData> existingRecords = 
                    partitionPredictionQueryService.queryPredictionByTime(
                        request.getYear(), request.getMonth(), request.getWeekSeq());
    ...
    public TotalActualDeliveryResponseDto calculateTotalActualDelivery(Integer year, Integer month, Integer weekSeq) {
        ...
        try {
            // 获取指定时间的数据
            List<CigaretteDistributionPredictionData> rawDataList = 
