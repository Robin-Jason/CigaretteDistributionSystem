...
│
├── event/                            # 应用层事件监听与统计
│   └── handler/                     # Spring Event 监听器
│       ├── DataImportEventHandler.java             # 导入流程事件监听器（日志与监控）
│       ├── DistributionPlanMonitorHandler.java     # 分配流程事件监听器（监控）
│       └── DistributionPlanStatisticsHandler.java  # 分配统计事件监听器（预留统计扩展）
│
└── dto/                              # 应用层DTO（数据传输对象）
...
```
domain/
├── event/                             # 领域/应用事件（基于 Spring Event 的应用内事件）
│   ├── DistributionPlanGenerationStartedEvent.java     # 分配方案生成开始事件
│   ├── DistributionPlanGenerationCompletedEvent.java   # 分配方案生成完成事件
│   ├── ExistingDataDeletedEvent.java                   # 现有分配数据删除事件
│   ├── DataImportStartedEvent.java                     # 数据导入开始事件
│   ├── DataImportCompletedEvent.java                   # 数据导入完成事件
│   └── DataImportFailedEvent.java                      # 数据导入失败事件
│
├── model/                             # 领域模型
...
- `application.event.handler` - 应用事件监听器
- `application.dto` - 数据传输对象
- `domain.event` - 领域/应用事件
- `domain.model` - 领域模型
