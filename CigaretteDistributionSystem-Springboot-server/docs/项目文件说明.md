...
| `PriceBandAllocationService.java`             | 价位段自选投放分配服务接口        | 定义按价位段自选投放（全市默认策略）的完整分配流程，包括初分配、价位段矩阵截断与微调、写回等     |
| `impl/PriceBandAllocationServiceImpl.java`    | 价位段自选投放分配服务实现        | 基于价位段候选卷烟列表，完成按价位段自选投放场景下的完整分配流程，写回 prediction_price 分区表   |
...
| 文件名                                                  | 职责定位                         | 主要功能简述                                                                                         |
|---------------------------------------------------------|----------------------------------|------------------------------------------------------------------------------------------------------|
| `DistributionWriteBackService.java`                     | 分配结果写回服务接口             | 定义统一的写回服务接口，支持标准分配和价位段分配两种写回策略                                         |
| `impl/StandardDistributionWriteBackServiceImpl.java`    | 标准分配写回服务实现             | 实现标准场景（按档位投放、按档位扩展投放等）的分配矩阵写回逻辑，支持多区域分配结果的写回            |
| `impl/PriceBandDistributionWriteBackServiceImpl.java`   | 价位段分配写回服务实现           | 实现价位段自选投放算法的分配结果批量写回，将结果写入 `cigarette_distribution_prediction_price` 分区表 |

---

### 2.11 `dto` / `converter` / `event.handler` 子层：应用内部 DTO、转换与事件处理
